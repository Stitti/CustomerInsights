...
import type { PainPointPoint, Trend } from "../mock/mockAnalytics";
...
export default function PainPointMatrix({ data, title = "Pain Points (Frequenz × Schwere)" }:
                                            { data: PainPointPoint[]; title?: string; }) {
    ...
    const options: ApexCharts.ApexOptions = {
        ...
        tooltip: {
            ...
            custom: ({ seriesIndex, dataPointIndex, w }) => {
                const p = w.config.series[seriesIndex].data[dataPointIndex] as any;
                const aspect = p.meta as string;
    ...
    return <Chart key={appearance} options={options} series={series} type="bubble" height={360} />;
}
