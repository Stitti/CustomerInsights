...
using CustomerInsights.WebhookService.Models;
using CustomerInsights.WebhookService.Utils;
...
namespace CustomerInsights.WebhookService.Services
{
    public class WebhookSenderService
    {
        ...
        public WebhookSenderService(HttpClient http, AsyncPolicy<HttpResponseMessage> policy, AppDbContext db, IOptionsSnapshot<WebhookOptions> opt)
        {
            _http = http;
            _policy = policy;
            _db = db;
        ...
        public async Task DispatchAsync(IncomingEvent ev, CancellationToken ct)
        {
            ...
            WebhookEndpoint[] endpoints = await _db.WebhookEndpoints
            ...
            foreach (var endpoint in endpoints)
            {
                ...
                WebhookDelivery delivery = new WebhookDelivery
                {
                    ...
                    PayloadJson = JsonConvert.SerializeObject(ev.Payload)
                };
        ...
        private static string BuildBody(WebhookDelivery d)
        {
            var envelope = new
            {
                ...
                data = JsonConvert.DeserializeObject<object>(d.PayloadJson)
            };
